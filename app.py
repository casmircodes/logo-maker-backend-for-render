...
keys = [os.environ.get("GOOGLE_API_KEY"), os.environ.get("GOOGLE_API_KEY1"), os.environ.get("GOOGLE_API_KEY2"), os.environ.get("GOOGLE_API_KEY3"), os.environ.get("GOOGLE_API_KEY4"), os.environ.get("GOOGLE_API_KEY5"), os.environ.get("GOOGLE_API_KEY6"), os.environ.get("GOOGLE_API_KEY7"), os.environ.get("GOOGLE_API_KEY8"), os.environ.get("GOOGLE_API_KEY9")]
...
def generate_images(prompt, num_images=4):
    ...
    for _ in range(num_images):
        try:
            time.sleep(2)  # Delay 10 seconds before each request
...
def generate_logo():
    ...
    try:
        ...
        # Build prompt
        prompt = f"I need a colorful traditional logo for my {industry} brand named {business_name}. Use matured and professional colors. Also make sure it is tempting and attractive to the eyes. Play with the brand name and the icon. White background. In {industry} industry logo style. Leverage 60, 30, 10 color principle. Make sure the concept of the logo icon is clear and meaningful. Remember on a white background. Do not add a slogan or tagline on it."
        if slogan.strip():
...
def generate_images(prompt, num_images=4):
    ...
    for _ in range(num_images):
        try:
            time.sleep(20)  # Delay 10 seconds before each request
